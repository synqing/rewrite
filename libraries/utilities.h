...
#include "global_defines.h"  // Make sure this contains the NUM_FREQS and NUM_TEMPI defines

#include "types.h"

// Fix ESP-IDF includes
#include <esp_system.h>  // Use angle brackets to search in system include paths

// REMOVE THESE INCORRECT PATHS
// #include "../components/esp-dsp/modules/math/include/dsps_math.h"
// #include "../components/esp-dsp/modules/fft/include/dsps_fft2r.h"
// #include "../components/esp-dsp/modules/iir/include/dsps_biquad.h"
// #include "../components/esp-dsp/modules/support/include/dsps_mem.h"

// ADD DSP FUNCTIONS PROPERLY
extern void* dsps_memcpy_aes3(void* dest, const void* src, size_t len);
extern void* dsps_memset_aes3(void* arr_dest, uint8_t set_val, size_t set_size);
extern esp_err_t dsps_mulc_f32_ae32(const float *input, float *output, int len, float C, int step_in, int step_out);
extern esp_err_t dsps_mulc_f32_ae32_fast(const float *input, float *output, int len, float C, int step_in, int step_out);
extern esp_err_t dsps_add_f32_ae32_fast(const float *input1, const float *input2, float *output, int len, int step);
extern esp_err_t dsps_mul_f32_ae32_fast(const float *input1, const float *input2, float *output, int len, int step);
...
extern float num_leds_float_lookup[];